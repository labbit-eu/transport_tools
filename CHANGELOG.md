...
- new tunnel-profile pruning integrated into stage 2 (input processing) via the `prune_tunnels` calculations setting (default `False`): per CAVER cluster and MD, a consensus exit distance is derived from a joint-transition score (radius expansion × survival drop, `prune_tunnels_mode` `'first'`/`'peak'`), each tunnel extending beyond the cut is classified as inflating into empty space and/or curved, and offensive tunnels are truncated in-place at a guarded cut (bottleneck-protected, water-radius floor). Only the sphere tails are removed; the refreshed `length`, layer membership and relevant-tunnel filter verdicts propagate downstream so stages 3+ see the pruned geometry. Tuning knobs live in the advanced settings (`prune_tunnels_bin_size`, `prune_tunnels_survival_perc_low/high`, `prune_tunnels_core_fraction_low/high`, `prune_tunnels_min_joint_threshold`, `prune_tunnels_eff_thresh`, `prune_tunnels_slope_percentile`, `prune_tunnels_water_radius`) and are part of the stage-2 SLURM shard fingerprint
...
- tunnel pruning now recomputes `Tunnel.length` and `Tunnel.curvature` from the kept sphere coordinates instead of leaving them stale after truncation: the sphere chain is anchored at the (transformed) average CAVER starting point — the same global-origin convention TransportTools uses for layer membership — and length is the arc length of the kept chain while curvature is `length / straight distance` to the last kept sphere. `cost`/`throughput`/`bottleneck_radius` are not recomputable from sphere data and keep their CAVER values (a logged limitation). The previous value pressed into `length` was the raw `length` CSV column, which does not equal the CAVER tunnel length, so this is also a correctness fix for the pruned geometry's downstream metadata
- tunnel pruning no longer assumes the CAVER `distance` column is monotonic: it is not (curved/wandering tunnels descend back toward the start — 405 of 8151 `md1` tunnels end below their own maximum). The truncation cut index is now the first sphere *at or after* the cut (`argmax(dists >= cut)`, clamped via `.any()`) instead of `np.searchsorted`, which is undefined on unsorted input, and per-tunnel maximum distances feeding the valid-region percentiles use `np.max(...)` instead of the last sphere — matching the very population (curved/wandering tails) the feature targets
- fix the tunnel-pruning water-radius floor off-by-one: the guarded cut now keeps *through* the first sphere at or after the guarded position whose radius meets the water floor, so the last kept sphere always satisfies the documented "minimum radius of the last kept sphere" contract (previously it kept one sphere short, leaving the last kept sphere below the floor whenever the floor loop advanced — and the dead `else start` fallback is gone). Tunnels with no floor-passing sphere past the guarded position are now left untouched instead of being cut one sphere short
- tunnel-pruning bottleneck protection now also keeps the profile sphere closest to the CAVER `bottleneck_radius` value, not only the argmin of the profile R column (a different sphere in general): since `bottleneck_radius` is deliberately not refreshed after truncation, the kept tunnel must still contain the sphere that value refers to — the "we never cut the bottleneck, so the CAVER value stays valid" invariant is now stated in the docstring and asserted before the cut is committed
- tunnel pruning now operates on the relevant-tunnel population only: the per-cluster consensus cut (and the post-cut inflation reference) is computed from `filters_passed == True` tunnels only, and only they are truncated — non-passing tunnels are left entirely untouched and no longer shift the consensus cut (previously, e.g. short stubs could pull the P10/P90 valid-range percentiles into the core region and distort the cut position). Clusters with fewer than two relevant tunnels have no consensus cut and are logged at debug level
