...
def _convert_msms_to_cgo(meshfile: str, color_id: int | None) -> List[float]:
    """
...
def msms_surface(msms_binary_path: str, spheres: List[Tuple[np.ndarray, float]], color_id: int | None,
    ...
    def _save_xyzr_file(spheres: List[Tuple[np.ndarray, float]], filename: str):
        """
    ...
    with tempfile.TemporaryDirectory(prefix="TT_") as tmp:
        ...
        os.close(fd1)  # Close the file descriptor, we just need the path

        _save_xyzr_file(spheres, xyzr_file)

        fd2, meshfile = tempfile.mkstemp(dir=tmp)
        os.close(fd2)
...
def filter_spheres(spheres: List[Tuple[np.ndarray, float]]) -> List[Tuple[np.ndarray, float]]:
    """
